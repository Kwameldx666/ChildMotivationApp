...
public sealed class RuleBasedAiOrchestrator(TimeProvider timeProvider) : IAiOrchestrator
{
    ...
    [
        new("Организуй рабочее место", "Разложи книги и тетради, протри стол, подготовь материалы на завтра.", "focus", ["учёба", "чистота"], 2, 80, 15, "Формирует привычку готовиться заранее."),
        new("Творческая пауза", "Создай мини-комикс о приключении семьи за 20 минут.", "creativity", ["рисование", "творчество"], 3, 110, 25, "Развивает воображение и речь."),
        new("Научный эксперимент", "Проведи маленький эксперимент: измерь, как быстро тает лёд в разных местах.", "science", ["наука", "эксперимент"], 3, 130, 30, "Поддерживает интерес к исследованиям."),
        new("Спортивный вызов", "Сделай 3 подхода упражнений на выбор: планка, приседания или прыжки.", "health", ["спорт", "здоровье"], 2, 90, 18, "Помогает выплеснуть энергию."),
        new("Миссия заботы", "Покорми питомца, обнови воду и добавь заметку о настроении.", "responsibility", ["дом", "уход"], 1, 60, 12, "Учит наблюдать и заботиться."),
        new("Математический блиц", "Реши 6 примеров на устный счёт или повтори таблицу умножения.", "learning", ["математика", "учёба"], 2, 95, 20, "Поддерживает уверенность в счёте."),
        new("Семейный репортёр", "Возьми интервью у родственника: " +
            "задай 3 вопроса о его дне и запиши ответы.", "communication", ["общение", "семья"], 2, 85, 16, "Прокачивает эмпатию и слушание."),
        new("Квест по дому", "Найди 5 предметов, которые лежат не на своих местах, и убери их.", "home", ["дом", "организация"], 1, 70, 14, "Быстрый способ вернуть порядок."),
        new("Мини-проект", "Составь список целей на неделю и укрась его наклейками.", "planning", ["планирование", "организация"], 3, 120, 28, "Создаёт ощущение контроля."),
        new("Дневник успеха", "Запиши 3 достижения за день и придумай награду за них.", "reflection", ["осознанность", "мотивация"], 1, 60, 10, "Формирует позитивный взгляд."),
    ];

    private static readonly RewardTemplate[] RewardTemplates =
    [
        new("Пикник в гостиной", "Импровизированный пикник с пледом и любимым десертом.", 320, "family", "🍓", "Подходит для выходного вечера."),
        new("Дополнительное время на творчество", "30 минут для творчества или конструктора.", 250, "creativity", "🎨", "Даёт пространство для самовыражения."),
        new("Совместная игра", "Родитель выбирает настольную игру вместе с ребёнком.", 280, "connection", "🎲", "Укрепляет семейную связь."),
        new("Приватный плейлист", "Ребёнок выбирает музыку на дорогу или обед.", 180, "music", "🎧", "Маленькая, но приятная привилегия."),
        new("Билет на мини-кино", "Выбор мультфильма с попкорном.", 420, "relax", "🎬", "Создаёт атмосферу праздника."),
        new("Карта добрых дел", "Сертификат на помощь от родителя: уборка, готовка или проект.", 350, "support", "🤝", "Показывает взаимную заботу."),
        new("Ночной фонарик", "Чтение сказки или фонарик для палатки под столом.", 200, "imagination", "🔦", "Подходит для младших детей."),
        new("Мини-шопинг", "Онлайн-выбор наклеек или аксессуара в условном бюджете.", 500, "shopping", "🛍️", "Учит управлять ресурсами."),
    ];

    public Task<TaskSuggestionsResponse> GenerateTaskSuggestionsAsync(TaskSuggestionsRequest request, CancellationToken cancellationToken)
    {
        ...
        if (ordered.Count == 0)
        {
            return Task.FromResult(TaskSuggestionsResponse.Empty());
        }
    ...
    public Task<RewardSuggestionsResponse> GenerateRewardSuggestionsAsync(RewardSuggestionsRequest request, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (request is null) throw new ArgumentNullException(nameof(request));
        ...
        var suggestions = RewardTemplates
            ...
            {
                Template = template,
                Score = ScoreTemplate(template.Tags, request.Interests, request.RecentlyPurchasedRewards, Array.Empty<string>()) +
    ...
    public Task<AiChatResponse> ProcessChatAsync(AiChatRequest request, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (request is null) throw new ArgumentNullException(nameof(request));
        if (string.IsNullOrWhiteSpace(request.Message)) throw new ArgumentException("Сообщение не может быть пустым.", nameof(request));
        ...
        return Task.FromResult(new AiChatResponse(conversationId, reply.ToString().Trim(), followUps, timeProvider.GetUtcNow()));
    }

    public Task<AiAnalyticsResponse> BuildAnalyticsAsync(AiAnalyticsRequest request, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (request is null) throw new ArgumentNullException(nameof(request));
    ...
    private static string BuildAnswerCore(string message)
    {
        ...
        {
            return "разбейте цель на короткие шаги, добавьте мгновенную награду и проговорите, какая выгода будет завтра.";
        }

        if (message.Contains("награ", StringComparison.OrdinalIgnoreCase))
        {
            return "смешайте быстрые микро-награды (наклейки, выбор музыки) и долгосрочные призы, чтобы видеть прогресс ежедневно.";
        }

        if (message.Contains("задач", StringComparison.OrdinalIgnoreCase))
        {
            return "задачи лучше объединять в тематические блоки и закрывать один блок за вечер, чтобы видеть результат.";
        }

        return "разбейте запрос на 3 шага: обсудите ожидания, договоритесь о правилах и закрепите итог коротким ритуалом.";
    }

    private static IReadOnlyCollection<string> BuildFollowUps(string message, IReadOnlyDictionary<string, string> context)
    {
    ...
    private static IEnumerable<AiInsightCard> GenerateInsights(int windowDays, int limit)
    {
        ...
        {
            new("warning", "Нужна пауза", "Ребёнок снижает активность к вечеру. Запланируйте лёгкие задания после 19:00.", "medium", ["routine", "energy"]),
            new("progress", "Сильная сторона", "Уровень вовлечения в учебные задания вырос на 18% за последние недели.", "success", ["learning", "focus"]),
            new("insight", "Экономика наград", "Средняя стоимость наград выросла. Добавьте больше бонусов до 200 очков.", "info", ["rewards", "budget"]),
            new("tip", "Командная цель", "Попробуйте семейную миссию на {windowDays} дней, чтобы закрепить новую привычку.", "info", ["family", "habits"])
        };
        ...
        IReadOnlyCollection<string> negativeMatches)
    {
        ...
        if (negativeMatches.Any(task => templateTags.Any(tag => task.Contains(tag, StringComparison.OrdinalIgnoreCase))))
        {
            score -= 1.5;
        }
        ...
        string MotivationHint)
    {
        ...
        private IReadOnlyCollection<string> TagsCache => _tags ??= Category.Split('-', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
