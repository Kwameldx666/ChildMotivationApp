...
public class NotificationServiceClient(IHttpClientFactory clientFactory, IOptionsSnapshot<NotificationEndpoints> endpoints)
    : INotificationServiceClient
{
    ...
    public Task<HttpResponseMessage> MarkAsReadAsync(string userId, List<string> notificationIds, CancellationToken cancellationToken = default)
    {
    ...
    public Task<HttpResponseMessage> DeleteAsync(string userId, Guid notificationId, CancellationToken cancellationToken = default)
    {
...
{
    public string Base { get; set; } = "/api/notifications";
}
