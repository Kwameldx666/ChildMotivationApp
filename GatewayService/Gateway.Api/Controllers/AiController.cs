...
using System.Linq;
using Gateway.Api.Contracts.Ai;
...
public sealed class AiController(IAiServiceClient aiClient) : ControllerBase
{
    ...
    public async Task<IActionResult> GetTaskSuggestions([FromBody] AiTaskSuggestionsRequest payload, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> GetRewardSuggestions([FromBody] AiRewardSuggestionsRequest payload, CancellationToken cancellationToken)
    {
        if (payload is null) return BadRequest("Request body cannot be null.");
    ...
    [HttpGet("analytics")]
    public async Task<IActionResult> GetAnalytics([FromQuery] AiAnalyticsQuery query, CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        if (string.IsNullOrWhiteSpace(userId)) return Unauthorized("User identifier is missing in the token.");

        var targetUserId = string.IsNullOrWhiteSpace(query.TargetUserId) ? userId : query.TargetUserId;
        using var response = await aiClient.GetAnalyticsAsync(targetUserId, query.FamilyId, query.WindowDays, query.MaxInsights, cancellationToken);
        return await response.ToActionResultAsync();
    }
}
