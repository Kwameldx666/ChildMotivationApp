using Gateway.Api.Contracts.Missions;
...
public class MissionsController(ITaskServiceClient taskClient) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateProgress(Guid id, [FromBody] UpdateMissionProgressRequest? request, CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        if (string.IsNullOrWhiteSpace(userId)) return Unauthorized("User identifier is missing in the token.");
