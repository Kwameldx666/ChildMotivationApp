using Gateway.Api.Contracts.Shop;
...
public class ShopProductsController(IShopServiceClient shopClient) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] UpsertProductRequest payload, CancellationToken cancellationToken)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpsertProductRequest payload, CancellationToken cancellationToken)
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
