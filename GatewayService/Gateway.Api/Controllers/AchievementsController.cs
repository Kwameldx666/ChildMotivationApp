using Gateway.Api.Contracts.Achievements;
...
public class AchievementsController(ITaskServiceClient taskClient) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateProgress(Guid id, [FromBody] UpdateAchievementProgressRequest? request, CancellationToken cancellationToken)
    {
        var userId = User.GetUserId();
        if (string.IsNullOrWhiteSpace(userId)) return Unauthorized("User identifier is missing in the token.");
