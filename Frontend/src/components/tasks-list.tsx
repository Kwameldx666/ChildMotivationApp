...
import { useTasks, useCompleteTask, useUpdateTask } from "@/services/tasks-queries"
...
const FILTERS: { id: TaskStatus | "all"; label: string; hint: string }[] = [
	...
	{ id: "pending", label: "Новые", hint: "Только что назначены" },
	{ id: "in_progress", label: "В работе", hint: "Ребёнок занимается" },
	{ id: "completed", label: "Готово", hint: "Подтверждённые" },
	{ id: "overdue", label: "Просрочено", hint: "Ждут внимания" },
]

const HERO_STATS = [
	{ id: "active", label: "Активные", hint: "В процессе или назначены", compute: (summary: Record<TaskStatus, number>) => summary.pending + summary.in_progress },
	...
	<div className="flex items-center gap-1">
		{[1, 2, 3, 4, 5].map((star) => (
			<Star
				...
				className={cn(
					"h-4 w-4",
					star <= level ? "text-amber-400 fill-amber-300" : "text-muted-foreground/20",
				)}
			/>
		))}
	</div>
...
function computeDifficulty(task: TaskDto): number {
	const base = (task.title?.length ?? 6) % 5
...
export default function TasksList({ userType }: TasksListProps) {
	...
	const decoratedTasks: DecoratedTask[] = tasks
		.map((task, index): DecoratedTask => {
			...
			const difficulty = computeDifficulty(task)
			const xpReward = 60 + difficulty * 25
	...
	const heroStats = useMemo(
		() => HERO_STATS.map((stat) => ({ ...stat, value: stat.compute(summary) })),
		[summary],
	)
	...
	const handleReject = async (task: TaskDto) => {
	...
	if (isError) {
		...
		if (typedError?.status === 401) {
			return <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm">Вы не авторизованы. Войдите, чтобы смотреть задачи.</div>
		}
		return (
			<div className="rounded-2xl border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm text-destructive">
				Ошибка при загрузке задач: {typedError?.message ?? "Попробуйте обновить страницу"}
			</div>
		)
	}
	...
	if (!tasks.length) {
		return (
			<div className="rounded-3xl border border-dashed border-primary/30 bg-primary/5 px-6 py-10 text-center shadow-sm">
				...
				<p className="mt-2 text-sm text-muted-foreground">Создайте первую задачу — и ребёнок увидит её в своём списке сразу после сохранения.</p>
				{userType === "parent" && (
					<Button
				...
				<div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
					<div className="space-y-6">
						...
						<p className="max-w-2xl text-sm text-white/80">
							Следите за темпом ребёнка, подтверждайте результаты и мягко напоминайте о важных задачах. Всё, что нужно, — в одном экране.
						</p>
					...
					<div className="rounded-3xl border border-white/20 bg-white/5 p-6 backdrop-blur">
						...
						<div className="mt-4 space-y-4">
							{highlighted.map((task) => (
								<div key={`highlight-${task.id}`} className="flex items-center justify-between gap-3 rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-sm">
									<div>
						...
						<div className="mt-5 rounded-2xl border border-dashed border-white/30 px-4 py-3 text-xs text-white/70">
							Совет: закрепите максимум 5 активных задач, чтобы ребёнку было легче держать темп.
						</div>
					</div>
				</div>
			</section>
							...
							<button
								...
								className={cn(
									...
									isActive
										...
										: "border-border/70 bg-card/70 text-muted-foreground hover:border-foreground/40",
								)}
							>
									...
									<div className="rounded-3xl border border-dashed border-border/60 bg-background/70 p-5">
										<div className="grid gap-3 sm:grid-cols-3">
											...
											<div className="rounded-2xl border border-primary/40 bg-primary/5 px-3 py-3">
												...
												<p className="mt-1 text-sm font-semibold text-primary">Фото или чек-лист</p>
											</div>
										</div>
