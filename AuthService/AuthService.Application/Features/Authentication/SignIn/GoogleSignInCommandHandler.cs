...
{
    public async Task<Result<GoogleSignInResult>> Handle(GoogleSignInCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!tokenResponse.IsSuccess)
        {
            ...
            logger.LogWarning("Google token endpoint failed: Status={Status}, Body={Body}", tokenHttpResponse.StatusCode, tokenRawBody);
        ...
        if (!string.IsNullOrWhiteSpace(tokenResponse.Value?.Error))
        {
            ...
            return Result.Failure<GoogleSignInResult>(HttpStatusCode.Unauthorized,
                AuthorizationErrors.Unauthorized($"Google OAuth error: {tokenResponse.Value.Error} - {tokenResponse.Value.ErrorDescription}"));
        }

        // Log token response metadata (don't log tokens themselves)
        logger.LogInformation("Google token response: HasAccessToken={HasAccessToken}, HasIdToken={HasIdToken}, TokenType={TokenType}, ExpiresIn={ExpiresIn}",
        ...
        {
            var userInfoHttpResponse = await googleServiceClient.RequestUserInfo(tokenResponse.Value!.AccessToken, cancellationToken);

            if (!userInfoHttpResponse.IsSuccessStatusCode)
            {
                var body = await userInfoHttpResponse.Content.ReadAsStringAsync();
                logger.LogWarning("Google userinfo request failed: Status={Status}, Body={Body}", userInfoHttpResponse.StatusCode, body);
            }
        ...
        else if (!string.IsNullOrWhiteSpace(tokenResponse.Value?.IdToken))
        {
            ...
            try
            {
                ...
                logger.LogInformation("Extracted user info from id_token: EmailSet={HasEmail}, NameSet={HasName}", !string.IsNullOrWhiteSpace(email), !string.IsNullOrWhiteSpace(name));
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Failed to decode id_token from Google token response.");
                return Result.Failure<GoogleSignInResult>(HttpStatusCode.BadRequest, DefaultErrors.BadRequest("Failed to obtain userinfo from Google token response."));
            }
        }
        else
        {
            return Result.Failure<GoogleSignInResult>(HttpStatusCode.BadRequest, DefaultErrors.BadRequest("No access token or id token was returned by Google."));
        }
